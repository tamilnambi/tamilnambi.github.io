...
document.getElementById("search").addEventListener("keyup", getDynamicSearch);

function getDynamicSearch(event) {
  ...
  fetch(
    ...
    .then(displayDynamicItems)
    .catch((err) => console.error(err));

  if (event.key === "Enter") {
    startSearchByBar(searchItem);
  }
}
function displayDynamicItems(response){
    let print="";
    let searchItems = [];
  for (let i = 0; i < 10; i++) {
...
function toggleSearchBar() {
  if (showSearchBar == false) {
    ...
    document.getElementById(
      "search-btn"
    ).innerHTML = `<i class="bi bi-x-lg text-light"></i>`;
    showSearchBar = true;
    getTrending();
  } else {
    ...
    document.getElementById(
      "search-btn"
    ).innerHTML = `<i class="bi bi-search text-primary"></i>`;
    showSearchBar = false;
  }
}
