...
function getResults(){
    ...
    const options = {
      ...
      fetch(`https://api.themoviedb.org/3/search/multi?query=${searchItem}&include_adult=false&language=en-US&page=1`, options)
...
function displayPeople(){
    ...
    for(let i in peopleList)
    {
        if(peopleList[i].profile_path == null)
            ...
            url = `https://image.tmdb.org/t/p/w500${peopleList[i].profile_path}`;
...
function convertDateFormat(dateToConvert){
    ...
    const m = [
      ...
      const newDate = day.getDate() + " " + m[day.getMonth()] + " " + day.getFullYear();
      return newDate;
}
